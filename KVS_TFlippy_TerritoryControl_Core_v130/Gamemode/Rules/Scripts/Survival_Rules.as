	...
	if (sleepers != null && sleepers.length > 0)
	{
		...
		for (u32 i = 0; i < sleepers.length; i++)
		{
			...
			if (sleeper !is null && !sleeper.hasTag("dead") && sleeper.get_bool("sleeper_sleeping") && sleeper.get_string("sleeper_name") == name)
			{
				...
				//KVSES-tcpr("[MISC] "+playerName + " joined, respawning him at sleeper " + sleeper.getName());
			}
		}
	}
...
void onPlayerDie(CRules@ this, CPlayer@ victim, CPlayer@ attacker, u8 customData)
{
	...
	if (isServer())
	{
		//KVSES-tcpr("[PDB] "+victimName +" has been killed by " + attackerName + "; damage type: " + customData);
				...
				if (isNeutral)
				{
					...
					if (default_respawn)
					{
						...
						if (ruins_count > 0)
						{
							...
							//KVSES-tcpr("[CCC] Chicken Chance: " + (chicken_chance * 100) + "%");
